...
> 接下来，让我们通过摊销分配（amortizing allocation）来提高速度。摊销分配是一种技术，它只有一次(或很少)分配，然后尝试复用它，而不是创建额外的分配。在前面的例子中，我们使用了由 CSV reader 上的 records 和 byte_records 方法创建的迭代器。这些迭代器为它产生的每一项分配新的记录空间，一条记录对应于一个分配。之所以这样做，是因为迭代器不能产生借用迭代器本身的项，而且创建新的分配往往更方便。
