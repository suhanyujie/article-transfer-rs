...
> 作为程序员，你将选择对 `pattern` 和 `count` 使用哪种 uint 类型 -- 解析器对各种 uint 类型都通用。我个人只会使用适合该值的最小单位。因此，示例 1 的样本适合 `u8`，示例 2 的模式适合于 u16，而在这两种情况下，计数都适用 `u8`。所以我会按照这样的原则使用。但我认为这不是很重要。
