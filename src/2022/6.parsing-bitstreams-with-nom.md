...
> 我首先想到的是“用bool” —— 您可能也在这么想。只需将 0 表示为 `false`，将 1 表示为 `true`。这肯定有效，但有点浪费。Rust 的 bool 类型实际上占用一个字节。因此，您可以表示比特位流为 `&[bool]`，但它将占用所需空间的 8 倍多字节！这对于一些应用程序来说都是可以，但是 Nom 可以更高效。
...
> 表示位流的解决方案很巧妙：我们使用两个值。首先是一个字节列表，其次是一个数字，表示下一个字节的具体位置。
...
> 这对值表示一个位流。第一个元素是比特流，每次存储一个字节（所以，8位一组）。第二个元素是偏移量，显示接下来应该读取哪一位（不是字节！）
...
> 例如，假设我们想要解析一个 16 位的序列，比如 `1111 0000 1100 1100`。我们从第一个元素位开始。下面展示了我如何表示位流和偏移量：
...
> 注意，这个小箭头指向输入的第一个比特。为了使用 `I = (&[u8], usize)` 类型表示这一点，我们将位输入分解为字节，以及当前指向的位的索引。如下所示：
...
> 因为第一个字节将永远不会再次使用，所以 Nom 通过删除第一个字节进行优化，并调整偏移量来考虑这一点:
...
> 这个解析器听起来很简单，但是调用它需要一点工作，因为它有很多泛型[1]，而 Rust 通常不能推断出这个类型。我总是提供特定具体类型的辅助函数。那么，让我们构建一个解析四个比特的解析器。一个 4 位的数字被称为 _nibble_ （因为它是半个字节……得到它…以工程标准来看，这简直就是个“笑话”）。
...
> 显然，它的 I（通用输入类型）将是标准的 `BitInput` 别名。但是 O（通用输出类型）是什么呢？毕竟，Rust 没有 u4 类型。我们如何存储一个 4 位的数字？
...
> 这个问题实际上只是早期的问题，即“我们如何在 Rust 中表示比特位”。看，我告诉过你在这篇文章里它出现了好几次。
...
> 按位解析的 [tag](https://docs.rs/nom/7.1.0/nom/bits/complete/fn.tag.html) 解析器匹配来自输入的特定位模式，如“0110”。同样，这是一个简单的想法，但它提出了一个棘手的问题：我们如何表示位的模式？是的，这是我第三次问“Rust 中我们如何表示比特位”。我告诉过你，这是一个很基本的问题！这里使用了一个非常相似的解决方案。
...
> 作为程序员，您将选择对 `pattern` 和 `count` 使用哪种 uint 类型 -- 解析器对各种 uint 类型都通用。我个人只会使用适合该值的最小单位。因此，示例 1 的样本适合 `u8`，示例 2 的模式适合于 u16，而在这两种情况下，计数都适用 `u8`。所以我会按照这样的原则使用。但我认为这不是很重要。
...
> 好了，现在我们知道了如何表示位数据，tag 解析器就很容易解析了。您提供一个位数据，Nom 逐位将其与输入位进行比较。像所有的解析器一样，tag 返回一个结果。如果解析器与输入匹配，则返回 Ok 和一对数据：（剩余的 input，匹配的 output）。如果没有匹配项，则返回 Err。因此，假如，解析比特流 10100 上的样本 101 时，将返回 `(00, 101)`。
...
> 记住，解析器组合器库的整个思想是：从几个小解析器开始，然后将它们与组合器函数组合在一起。我们已经看到了两个基本的“位流”解析器，tag 和 take。以下是如何将它们与我上一篇帖子中的 [map](https://docs.rs/nom/7.1.0/nom/combinator/fn.map.html) 组合器相结合：
...
> 到目前为止，我们已经学习了如何制作简单的位解析器，并将它们组合成复杂的解析器。我们甚至已经了解了 Nom 如何表示比特流。最后一个问题是：这些比特流到底是从哪里来的？毕竟，大多数 Rust 函数以字节表示二进制数据（例如，`Vec<u8>` 或使用 bytes crate）。如果您从磁盘、RAM 或网络读取二进制数据，那么几乎可以肯定它将以字节存储。因此，我们需要一种将字节流转换为比特流的方法。
