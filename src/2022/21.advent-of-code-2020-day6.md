>* Advent of Code 2020 Day6 译文（用 Rust 实现 Advent of Code 2020 第5天）
...
离 [Advent of Code 2020](https://adventofcode.com/2020/) 结束，但我们还有很长一段路要走。第六天的时间到了。
...
因此，第一部分是为组（group）中的每个人收集结果 —— 我们甚至不需要解析器！
...
但不幸的是，它计算的是两个 `HashSet` 之间的联合，并返回一个迭代器。
...
Amos：是的，很聪明! 这个库的作者和我们最近研究的智能字符串是同一个作者。
...
酷熊：所以我们只是... 我们只是使用 `im::HashSet::unions` 来 `map` 我们的 group？
...
每个组（group）都有一定数量的“yes”(“yes”表示在集合中有一个字母)。

题目要求计算所有小组回答的所有问题的总和。
...
酷熊：等等，用 `unwrap_or_default` ? 为什么？

Amos：那么，如果我们 `fold1` 一个 0 项的集合会怎么样呢? 那么开始就没有 `init` 值了！
