...
1.  **Parse the CLI args using [picoargs](https://crates.io/crates/pico-args)**
...
2.  **Serialize the DNS query using [bitvec](https://docs.rs/bitvec), an _awesome_ general-purpose crate for reading or writing individual bits**
...
3.  **Communicate with a DNS resolver with the stdlib [UdpSocket](https://doc.rust-lang.org/stable/std/net/struct.UdpSocket.html) type**

    I had no idea how this worked, but the Rust stdlib is really well-documented, so I was sure I could pick it up.

4.  **Parse the binary response with [Nom](https://docs.rs/nom)**
