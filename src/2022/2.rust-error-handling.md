...
在本文中，我打算详细讲解 Rust 中如何处理错误。更重要的是，我将尝试每次讲解一个错误处理部分，这样你会知道，如何将所有部分组合在一起使用。
...
如果天真地这样做，Rust 中的错误处理可能是冗长和烦人的。本文将探索这些障碍，并演示如何使用标准库使错误处理简洁和符合人体工程学。
...
目标受众: 那些初学者还不知道其处理习惯用法的错误。对 Rust 稍微熟悉一下是有帮助的。(本文大量使用了一些标准的 traits，以及一些非常轻量级的闭包和宏。)

Update (2018/04/14): Examples were converted to ?, and some text was added to give historical context on the change.

更新(2018/04/14) : 示例被转换为? ，并添加了一些文本以提供更改的历史背景。
...
锈书有一个关于错误处理的章节。它提供了一个非常简短的概述，但是(还)没有进行足够详细的介绍，特别是在处理标准库中的一些最近添加的内容时。

Run the code!
运行代码
If you’d like to run any of the code samples below, then the following should work:

如果你想运行下面的任何一个代码示例，那么下面的代码应该可以:
...
每个代码示例都带有其名称的标签。(没有名称的代码示例不能以这种方式运行。对不起。)
...
If you’re new to Rust, systems programming and expressive type systems, then start at the beginning and work your way through. (If you’re brand new, you should probably read through the 如果你对 Rust、系统编程和表达型系统是新手，那么从头开始，一步一步来。(如果你是全新的，你可能应该通读一下Rust book 生锈的书 first.) 第一。)
If you’ve never seen Rust before but have experience with functional languages (“algebraic data types” and “combinators” make you feel warm and fuzzy), then you can probably skip right over the basics and start by skimming 如果你以前从未见过 Rust，但对函数式语言(“代数数据类型”和“组合子”让你感到温暖和模糊)有经验，那么你可以跳过这些基础知识，先浏览一下multiple error types 多种错误类型, and work you’re way into a full read of 你已经完全理解了standard library error traits 标准程序库错误特性. (Skimming 。(略读the basics 最基本的 might be a good idea to just get a feel for the syntax if you’ve really never seen Rust before.) You may need to 如果您以前真的从未见过 Rust，那么了解一下语法可能是个好主意。)你可能需要consult the Rust book 查阅锈书 for help with Rust closures and macros. 用于帮助防锈封闭和宏
If you’re already experienced with Rust and just want the skinny on error handling, then you can probably skip straight 如果您已经有了 Rust 方面的经验，并且只是想了解错误处理的细节，那么您可以直接跳过to the end 直到最后. You may find it useful to skim the 。你可能会发现浏览case study 个案研究 for examples. 例如
The Basics 基本知识
Unwrapping explained 解包
The 这个Option type 类型
Composing 作曲Option<T> values 价值观
The 这个Result type 类型
Parsing integers 解析整数
The 这个Result type alias idiom 类别成语
A brief interlude: unwrapping isn’t evil 一个简短的插曲: 打开并不是坏事
Working with multiple error types 处理多种错误类型
Composing 作曲Option and 及Result
The limits of combinators 组合子的极限
Early returns 提前返回
The 这个try! macro/ 宏/? operator 操作员
Defining your own error type 定义自己的错误类型
Standard library traits used for error handling 用于错误处理的标准库特性
The 这个Error trait 特征
The 这个From trait 特征
The real 真实的try! macro/ 宏/? operator 操作员
Composing custom error types 编写自定义错误类型
Advice for library writers 给图书馆作家的建议
Case study: A program to read population data 案例研究: 读取人口数据的程序
It’s on Github 在 Github 上
Initial setup 初始设置
Argument parsing 参数解析
Writing the logic 写逻辑
Error handling with 错误处理Box<Error>
Reading from stdin 从 stdin 中读取
Error handling with a custom type 使用自定义类型进行错误处理
...
保持代码的可组合性非常重要，因为如果没有这个需求，我们可能会在遇到意想不到的事情时感到恐慌。(恐慌导致当前任务展开，在大多数情况下，整个程序中止。)下面是一个例子:
...
如果给这个程序零个参数(错误1) ，或者如果第一个参数不是整数(错误2) ，那么程序就会像第一个示例一样陷入混乱。
...
我喜欢把这种错误处理方式想象成一头公牛在瓷器店里奔跑。公牛会到达它想去的地方，但是在这个过程中它会践踏一切。
...
在前面的示例(unwrap-double)中，我声称如果程序达到两个错误条件中的一个，它将简单地感到恐慌，然而，该程序没有像第一个示例(panic-simple)那样包含显式的感到恐慌的调用。这是因为这种恐慌嵌入在要求打开的调用中。
...
“打开”Rust 中的某些内容是说，“给我计算结果，如果有错误，只要惊慌并停止程序。”如果我只显示用于展开的代码会更好，因为它非常简单，但是要做到这一点，我们首先需要探索 Option 和 Result 类型。这两种类型都有一个名为 unwrap 的方法。
...
Option 类型是用 Rust 类型系统来表示缺席的可能性的一种方式。将缺席的可能性编码到类型系统中是一个重要的概念，因为它将导致编译器强制程序员处理缺席的情况。让我们来看一个在字符串中寻找字符的例子:
...
(专业提示: 不要使用此代码，而是使用标准库中的 find 方法。)
...
注意，当这个函数找到匹配的字符时，它不仅仅返回偏移量。相反，它返回 Some (偏移量)。有些是 Option 类型的变体或值构造函数。你可以把它看作一个类型为 fn < t > (值: t)-> Option < t > 的函数。相应地，None 也是一个值构造函数，只是它没有参数。您可以将 None 看作是 fn < t > ()-> Option < t > 类型的函数。
...
这段代码使用模式匹配来对 find 函数返回的 Option < usize > 进行案例分析。实际上，实例分析是获取 Option < t > 中存储的值的唯一方法。这意味着，作为程序员，当 Option < t > 是 None 而不是 Some (t)时，必须处理这种情况。
...
但是等等，用 unwrap-double 怎么样？那里没有案例分析！相反，案例分析被放在 unwrap 方法中。如果你愿意，你可以自己定义它:
...
Unwrap 方法抽象出了案例分析。这正是使打开包装的人体工程学使用的东西。不幸的是，这种恐慌！意味着拆开的包装是不可组合的: 它是瓷器店里的公牛。
...
在 option-ex-string-find 中，我们看到了如何使用 find 来发现文件名中的扩展名。当然，并非所有文件名都有。所以文件名可能没有扩展名。这种缺席的可能性被编码到使用选项 < t > 的类型。换句话说，编译器将强迫我们处理扩展不存在的可能性。在我们的例子中，我们只是打印出一条消息说明这一点。
...
代码保持简单，但需要注意的重要事情是，find 类型迫使我们考虑缺席的可能性。这是一件好事，因为这意味着编译器不会让我们意外地忘记文件名没有扩展名的情况。另一方面，像我们每次在 extension _ explicit 中所做的那样进行明确的案例分析可能会让人有点厌烦。
...
实际上，extension _ explicit 中的案例分析遵循一个非常常见的模式: 将一个函数映射到 Option < t > 中的值，除非选项是 None，否则只返回 None。

Rust has parametric polymorphism, so it is very easy to define a combinator that abstracts this pattern:

锈有参数多态，所以很容易定义一个组合子来抽象这种模式:
...
我发现的另一个非常常见的模式是，当 Option 值为 None 时，为案例分配一个默认值。例如，可能您的程序假设文件的扩展名是 rs，即使没有。正如你可能想象的那样，这方面的案例分析并不是针对文件扩展名的ーー它可以用于任何选项 < t > :

option-unwrap-or 选项-展开-或
...
(请注意 unwrap _ or 在标准库中被定义为 Option < t > 上的一个方法，因此我们在这里使用它来代替上面定义的独立函数。不要忘记查看更一般的 unwrap _ or _ else 方法
...
我认为还有一个值得特别关注的组合子: and then。它使得组成独特的计算，承认缺席的可能性变得容易。例如，本节中的大部分代码都是关于查找给定文件名的扩展名的。为此，首先需要从文件路径提取的文件名。虽然大多数文件路径都有文件名，但并非所有路径都有文件名。例如:。，..或/。

So, we are tasked with the challenge of finding an extension given a file path. Let’s start with explicit case analysis:

因此，我们的任务就是找到一个给定文件路径的扩展。让我们从明确的案例分析开始:
...
您可能认为我们可以使用映射组合器来减少案例分析，但是它的类型并不完全适合。也就是说，map 接受一个只对内部值执行某些操作的函数。函数的结果总是用 Some 重新包装。相反，我们需要类似 map 的东西，但是它允许调用者返回另一个 Option。它的通用实现甚至比 map 更简单:

option-and-then 选择-然后
...
现在我们可以重写 file _ path _ ext 函数，而不需要进行明确的案例分析:
...
Option 类型在标准库中定义了许多其他的组合子。浏览一下这个列表，熟悉一下可以找到的东西是个不错的主意——它们通常可以帮你减少案例分析。熟悉这些组合子将带来好处，因为它们中的许多也是为 Result 定义的(具有类似的语义) ，我们将在下面讨论。
...
组合子使用类似选项人体工学的类型，因为他们减少了明确的案例分析。它们也是可组合的，因为它们允许调用者以自己的方式处理缺席的可能性。像 unwrap 这样的方法移除选项，因为如果 Option < t > 为 None，它们会感到恐慌。
...
Result 类型是 Option 的更丰富版本。结果表达了错误的可能性，而不是像选项那样表达缺席的可能性。通常，错误被用来解释为什么一些计算的结果失败。这是一个严格的更一般的选择形式。考虑下面的类型别名，它在语义上与实选项 < t > 在各个方面都等价:

option-as-result 结果选择
...
这修复了 Result 的第二个类型参数 always be ()(发音为“ unit”或“ empty tuple”)。只有一个值存在于()类型中: ()。(是的，类型和值级别的术语有相同的符号!)
...
Result 类型是表示计算中两种可能结果之一的一种方式。按照惯例，一个结果意味着预期或“ Ok”，而另一个结果意味着意外或“ Err”。

Just like Option, the Result type also has an unwrap method defined in the standard library. Let’s define it:

和 Option 一样，Result 类型也有一个在标准库中定义的 unwrap 方法:
...
这实际上与我们对 Option: : unwrap 的定义相同，只不过它在 panic 中包含了错误值！信息。这使得调试更加容易，但是它也要求我们在 e 类型参数(表示我们的错误类型)上添加 Debug 约束。由于绝大多数类型都应该满足 Debug 约束，因此在实践中往往可以做到这一点。(对类型进行调试仅仅意味着有一种合理的方法可以使用该类型打印人类可读的值描述。)
...
Rust 标准库使字符串转换为整数变得非常简单。事实上，它是如此简单，以至于写下这样的东西是非常诱人的:

result-num-unwrap 结果-数字-展开
...
在这一点上，您应该对调用 unwrap 持怀疑态度。例如，如果字符串没有被解析为一个数字，你会得到一个恐慌:
...
这是相当不雅观的，如果这发生在您正在使用的库中，您可能会被惹恼，这是可以理解的。相反，我们应该尝试处理函数中的错误，让调用者决定要做什么。这意味着更改 double _ number 的返回类型。但是为了什么？这需要查看标准库中 parse 方法的签名:
...
嗯。所以我们至少知道我们需要使用 Result。当然，这也有可能返回一个选项。毕竟，一个字符串要么作为数字解析，要么不作为数字解析，对吗？这当然是一种合理的方式，但是实现在内部区分了为什么字符串没有被解析为整数。(无论是空字符串、无效数字、太大或太小。)因此，使用 Result 是有意义的，因为我们希望提供更多的信息，而不仅仅是“缺席”我们想说明解析失败的原因。当面临选项和结果之间的选择时，您应该试着模仿这种推理方式。如果您可以提供详细的错误信息，那么您可能应该这样做。(我们稍后会看到更多内容。)
...
好的，但是我们如何写返回类型呢？上面定义的解析方法对于标准库中定义的所有不同数字类型都是泛型的。我们也可以(也许应该)使我们的函数具有通用性，但是让我们先明确一下。我们只关心 i32，因此需要找到它的 FromStr 实现(在浏览器中为“ FromStr”执行 CTRL-F) ，并查看它的相关类型 Err。我们这样做是为了找到具体的错误类型。在本例中，它是 std: : num: ParseIntError。最后，我们可以重写函数:

result-num-no-unwrap 结果-数量-没有-打开
...
这是一个更好的一点，但现在我们已经编写了更多的代码！案例分析再次让我们感到困惑。
...
用组合数来拯救！和 Option 一样，Result 也有很多定义为方法的组合子。结果和选项之间有一个很大的公共组合子的交集。特别是，map 是这个十字路口的一部分:
...
最常见的疑问都是关于 Result 的，包括 unwrap _ or 和 then。此外，由于 Result 有第二个类型参数，因此存在仅影响错误类型的组合子，例如 map _ err (而不是 map)和 or _ else (而不是 and _ then)。
...
在标准库中，您可能经常看到 Result < i32 > 这样的类型。但是等等，我们定义了 Result 有两个类型参数。我们怎样才能仅仅指定一个呢？关键是定义一个 Result 类型别名，该别名将类型参数之一修复为特定类型。通常固定类型是错误类型。例如，我们之前的解析整数的例子可以这样重写:
...
我们为什么要这么做？好吧，如果我们有很多函数可以返回 ParseIntError，那么定义一个总是使用 ParseIntError 的别名会更方便，这样我们就不必一直写出来了。
...
这个习惯用法在标准库中最突出的地方是用 io: : Result。通常情况下，写 io: : Result < t > ，这清楚地表明您使用的是 io 模块的类型别名，而不是 std: : Result 中的简单定义。(这个成语也用来形容 fmt: : Result.)
...
如果您一直在跟踪，您可能已经注意到，我对像 unwrap 这样的调用方法采取了相当强硬的措施，因为它可能会导致错误并中止您的程序。一般来说，这是个好建议。
...
然而，展开仍然可以明智地使用。使用 unwrap 的确切理由是一个灰色地带，理性的人可能不同意。我将总结一下我对这个问题的一些看法。

In examples and quick ‘n’ dirty code. 在示例和快速 n 脏代码中 Sometimes you’re writing examples or a quick program, and error handling simply isn’t important. Beating the convenience of 有时候你需要编写示例或者一个快速的程序，错误处理并不重要unwrap can be hard in such scenarios, so it is very appealing. 在这种情况下是很困难的，所以很有吸引力
When panicking indicates a bug in the program. 当恐慌表明程序中有错误 When the invariants of your code should prevent a certain case from happening (like, say, popping from an empty stack), then panicking can be permissible. This is because it exposes a bug in your program. This can be explicit, like from an 当代码的不变量应该能够防止某种情况发生(比如从空堆栈中弹出)时，就可以允许恐慌。这是因为它暴露了程序中的一个 bug。这可以是明确的，比如从assert! failing, or it could be because your index into an array was out of bounds. 失败，也可能是因为数组的索引超出了界限
...
这可能不是一个详尽的清单。此外，在使用 Option 时，最好使用其 expect 方法。Expect 和 unwrap 的功能完全一样，只是它会打印一条你想要的消息。这使得产生的恐慌更容易处理，因为它将显示您的消息而不是“ unwrap on a None 值”
...
我的建议可以归结为: 做出正确的判断。这就是为什么“永远不要做 x”或者“ y 被认为是有害的”这样的字眼没有出现在我的文章中的原因。所有的事情都需要权衡，作为程序员的您应该决定什么是您的用例可以接受的。我的目标只是帮助你尽可能准确地评估权衡。
...
现在我们已经介绍了 Rust 中错误处理的基本知识，并且已经介绍了关于展开的部分，让我们开始探索更多的标准库。
...
到目前为止，我们已经研究了错误处理，其中所有内容都是 Option < t > 或 Result < t，SomeError > 。但是当你同时拥有一个选项和一个结果时会发生什么呢？或者如果你有一个结果 < t，Error1 > 和一个结果 < t，Error2 > 怎么办？处理不同错误类型的组合是摆在我们面前的下一个挑战，也是本文余下部分的主要内容。
...
到目前为止，我已经讨论了为 Option 定义的组合子和为 Result 定义的组合子。我们可以使用这些组合子来组合不同计算的结果，而不需要进行显式的实例分析。
...
当然，在真正的代码中，事情并不总是那么干净。有时你有一个选项和结果类型的混合。我们必须进行明确的案例分析，还是继续使用组合子？
...
考虑到我们新发现的关于 Option、 Result 和它们的各种组合子的知识，我们应该尝试重写它，以便正确地处理错误，并且如果出现错误，程序不会恐慌。
...
这里比较棘手的一点是，argv.nth (1)生成一个 Option，而 arg.parse ()生成一个 Result。这些不是直接可组合的。当同时面对选择权和结果权时，解决方案通常是将选择权转换为结果。在我们的示例中，缺少命令行参数(来自 env: : args ())意味着用户没有正确地调用程序。我们可以用一个 String 来描述这个错误。让我们试试:

error-double-string 错误-双字符串
use std::env;
...
在这个例子中有一些新的东西。第一个是使用选项: : ok 或者组合符。这是将一个选项转换为一个结果的一种方法。如果 Option 为 None，则转换要求您指定要使用的错误。和我们见过的其他组合子一样，它的定义非常简单:

option-ok-or-def 选择 ok 还是 def
...
这里使用的另一个新的组合符是 Result: : map _ err。这就像 Result: : map，只不过它将一个函数映射到 Result 值的错误部分。如果 Result 是 Ok (...)值，则返回未修改的值。
...
我们在这里使用 map _ err 是因为错误类型必须保持不变(因为我们使用了 and _ then)。由于我们选择将 Option < String > (从 argv.nth (1))转换为 Result < String，String > ，所以我们还必须将 ParseIntError 从 arg.parse ()转换为 String。
...
让我们从简单的开始。我们的任务是打开一个文件，读取其所有的内容，并将其内容转换为一个数字。然后乘以2，输出结果。
...
尽管我已经尝试说服您不要使用 unwrap，但是首先使用 unwrap 编写代码还是很有用的。它使您能够专注于您的问题，而不是错误处理，并且它公开了需要发生适当错误处理的地方。让我们从这里开始，这样我们就可以得到代码的句柄，然后重构它以使用更好的错误处理。

io-basic-unwrap 基本打开
use std::fs::File;
use std::io::Read;
use std::path::Path;
...
(注意:。使用 AsRef < path > 是因为这些边界与 std: : fs: : File: : open 上使用的边界相同。这使得使用任何类型的字符串作为文件路径成为可能。)

There are three different errors that can occur here:

这里可能会出现三种不同的错误:

A problem opening the file. 打开文件有问题
A problem reading data from the file. 从文件中读取数据有问题
A problem parsing the data as a number. 将数据解析为一个数字的问题
...
前两个问题通过 std: : io: Error 类型描述。我们知道这一点是因为 std: : fs: : File: : open 和 std: : io: : Read: Read _ to _ string 的返回类型。(请注意，它们都使用前面描述的 Result 类型别名惯用法。如果单击 Result 类型，您将看到类型别名，从而看到底层 io: : Error 类型。)第三个问题由 std: : num: : ParseIntError 类型描述。错误类型在整个标准库中都很普遍。你会一次又一次地看到它。
...
让我们开始重构 file _ double 函数的过程。为了使这个函数与程序的其他组件可组合，如果满足上述任何错误条件，不应该惊慌。实际上，这意味着如果函数的任何操作失败，该函数应该返回一个错误。我们的问题是 file _ double 的返回类型是 i32，这不能给我们提供任何有用的报告错误的方法。因此，我们必须首先将返回类型从 i32更改为其他类型。
...
我们需要决定的第一件事是: 我们应该使用选项还是结果？我们当然可以很容易地使用选项。如果出现这三个错误中的任何一个，我们可以简单地返回 None。这会起作用，这比惊慌失措要好，但我们可以做得更好。相反，我们应该传递一些关于发生的错误的细节。既然我们要表示出错的可能性，我们应该使用 Result < i32，e > 。但是 e 应该是什么呢？由于可能发生两种不同类型的错误，因此我们需要将它们转换为通用类型。其中一种类型是 String。让我们看看这对我们的代码有什么影响:
...
这段代码看起来有点复杂。在这样的代码变得容易编写之前，需要相当多的练习。我写它的方式是按照下面的类型。一旦我将 file _ double 的返回类型改为 Result < i32，String > ，我就必须开始寻找正确的组合子。在本例中，我们只使用了三个不同的组合子: and _ then、 map 和 map _ err。
...
然后用于链接多个计算，其中每个计算都可能返回一个误差。在打开文件之后，还有两个计算可能失败: 从文件中读取和将内容解析为一个数字。相应地，有两个 to 和 then 调用。
...
Map 用于将函数应用于 Result 的 Ok (...)值。例如，映射的最后一个调用将 Ok (...)值(即 i32)乘以2。如果在此点之前发生错误，则会跳过此操作，因为映射是如何定义的。
...
地图错误是使所有这一切工作的诀窍。Map _ Err 与 map 类似，只是它将一个函数应用于 Result 的 Err (...)值。在这种情况下，我们希望将所有错误转换为一种类型: String。由于 io: : Error 和 num: : ParseIntError 都实现了 ToString，我们可以调用 to _ string ()方法来转换它们。
...
尽管如此，代码仍然是毛茸茸的。掌握使用组合子很重要，但它们也有局限性。让我们尝试一种不同的方法: 早期回报。
...
我想从上一节中提取代码，并使用早期返回重写它。提前返回可以让您提前退出函数。我们不能在 file _ double 中从另一个闭包返回早期的值，所以我们需要回到显式的 case 分析。
...
理智的人可以争论这段代码是否比使用组合子的代码更好，但是如果你不熟悉组合子的方法，这段代码对我来说读起来更简单。它使用带有 match 和 if let 的显式案例分析。如果出现错误，它只是停止执行该函数并返回错误(通过将其转换为字符串)。
