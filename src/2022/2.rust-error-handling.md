...
因此，我们的任务就是找到指定的文件路径的扩展名。我们从显式的 case analysis 开始：
...
这种特殊的错误类型表示可能出现的两种类型错误：处理 I/O 错误或将字符串转换为数字的错误。通过向枚举定义中添加新的变体，一次类推，该错误可以表示任意多的错误类型。
...
我们已经非常接近理想的错误处理。对于错误处理，我们代码的开销很小，因为 `?` 操作符同时包含三个东西:
...
这篇文章很长，根据你的背景，它可能相当密集。虽然这篇文章中有大量的示例代码，但大部分都设计地有针对性。虽然我没有足够的智慧来制作专业的教学范例，但我可以写一个案例研究。
...
之前，我们开始重构代码，将函数的类型从 `T` 改为 `Result<T, OurErrorType>`。在这种情况下，`OurErrorType` 只是 `Box<Error>`。但 `T` 是什么？我们可以给 `main` 添加一个返回类型吗？
...
让我们将代码重构为结构体自己的函数，但是保留 unwrap 的调用。请注意，我们选择通过忽略该行来处理缺失人口计数的可能性。
...
现在我们已经了解了如何使用 `Box<Error>` 进行适当的错误处理，接下来让我们使用自定义错误类型尝试一种不同的方法。但是首先，让我们暂停一下错误处理，添加从 stdin 读取的支持。
...
我们所做的只是在 Docopt 使用字符串中使 `data-path` 参数可选，并使相应的结构成员 `arg_data_path` 可选。剩下的就交给 docopt crate 了。
...
使用自定义错误类型(就像我们在这里所做的)的一个有用的方面是，`main` 函数现在可以选择以不同的方式处理错误。在此之前，由于 `Box<Error>` 没有太多选择: 只能打印消息。我们现在仍然在这样做，但是如果我们想要，比如说，添加一个 `--quiet` 标志呢？`--quiet` 标志应该使任何冗长的输出保持沉默。
...
现在，如果程序没有找到匹配项，它将输出一条说明消息。这可能有点笨拙，特别是如果您打算在 shell 脚本中使用该程序。
...
让我们从添加参数标志开始。与前面一样，我们需要调整使用字符串并向 Args 结构添加一个标志。剩下的就交给 docopt 板条箱了:
...
这几乎总结了我们的案例研究。从这里开始，你应该准备好进入这个新世界了，用适当的错误处理编写自己的程序和库。
...
因为这篇文章很长，所以对 Rust 中的错误处理进行一个快速的总结是很有用的。这些是我的“经验法则”它们显然不是必须的。也可能有更好的方式打破这些总结！
