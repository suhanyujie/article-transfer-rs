>* Advent of Code 2020 Day1 译文（Advent of Code 2020 第一天）
...
今年 12 月我还没有具体的打算，但是我周围的很多人（在 Twitter 上，在工作中）都选择了这个 [Advent of Code](https://adventofcode.com/) 来学习 Rust，而且我有很大的 [FOMO](https://en.wikipedia.org/wiki/Fear_of_missing_out) 情绪，所以，看看最后我能走多远。
...
> 酷熊小贴士
> Quick JavaScript got cha: `parseInt` 接受多个参数，包括要解析的数字的基数（radix），因此我们不能只是将其传递给 map，后者将传递项和索引。
> 这个方法有效：
...
如果我们再次调用 `.unwrap()`，那么我们只得到普通的 `i64` 值，如下所示：
...
酷熊：哦，`todo!()` 就是要实现的吗？
Amos: 对！它允许我们将某些内容标记为“待完成”，并且它将进行编译（而不是抱怨我们没有返回正确的类型，如果我们让 `find_pair_whose_sum_is_2020` 函数体直接为空，就会发生这种情况）
酷熊：太棒了，但结尾为何是个 `!`？
...
（这段代码具有完全相同的输出 —— 现在我将停止显示 shell 会话。）
...
Amos: 对！
酷熊：但是我们的函数返回一个 ... `anyhow::Result<()>`，哪里有 `E`？
...
Amos: 你不能用一个迭代器来做这件事，因为一旦你检索到一个值，它就会从迭代器中移除。
