...
>如果我们不调用 `thread_rng()`，而是接受随机发生器（randomizer）的方式：
...
>...然后，我们就可以在测试中使用一个虚假的、**可预测**的 PRNG，而用户只需要使用他们真实场景中的 PRNG 即可。
...
>对所有地纯函数来说，尽职调查：从技术上讲，`do_something()` 和 `random()` 都不是纯函数，因为它们都缺少一个名为“引用透明度”（[referential transparency](https://en.wikipedia.org/wiki/Referential_transparency)）的属性。如果你检查这样做，就会这样：
