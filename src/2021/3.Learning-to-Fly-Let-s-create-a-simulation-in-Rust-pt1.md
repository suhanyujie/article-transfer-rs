...
>人工神经网络（我称之为神经网络）是著名的**概念**数据集（如学习一直猫的样子），所以他们发现他们使用面部识别（如相机），语言翻译（如 [GNMT](https://en.wikipedia.org/wiki/Google_Neural_Machine_Translation)），在我们的例子中引导彩色像素的 reddit 社区。
